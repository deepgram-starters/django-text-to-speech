...
Django demo app for Deepgram Text-to-Speech.
...
| File | Purpose |
|------|---------|
| `starter/views.py` | Main backend — API endpoints and request handlers |
...
4. Handle it in the backend `starter/views.py` — read the param and pass it to the Deepgram API
...
```
feat(django-text-to-speech): add diarization support
fix(django-text-to-speech): resolve WebSocket close handling
refactor(django-text-to-speech): simplify session endpoint
